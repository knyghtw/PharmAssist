...
export default class stokService {
  ...
  static async getItems() {
    try {
      ...
      const result = await db.select("SELECT * FROM stok_obat");
      return result;
    } catch (error) {
  ...
  static async createItem(
    ...
    try {
      ...
      const existingData = await db.select(
        "SELECT id_obat, no_batch FROM stok_obat WHERE id_obat = $1 AND no_batch = $2",
        [id_obat, no_batch]
      );

      if (existingData.length > 0) {
        throw new Error(
          `Stok obat dengan id_obat "${id_obat}" dan no_batch "${no_batch}" sudah ada`
        );
      }

      const result = await db.execute(
        "INSERT INTO stok_obat (id_obat, id_pbf, no_batch, harga_beli_per_satuan, harga_jual_per_satuan, tanggal_expired, jumlah_stok) VALUES ($1, $2, $3, $4, $5, $6, $7)",
        [
          id_obat,
          id_pbf,
          no_batch,
